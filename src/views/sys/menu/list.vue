<template>
  <div class="app-container">
    ...
    <el-table
      ...
      border
      default-expand-all
      :tree-props="{ children: 'children', hasChildren: 'hasChildren' ,ref:'menuTree'}"
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="scope.row.level == 1 && hasPerm('permission.add')"
            type="text"
            size="mini"
            @click="
              () => {
    ...
    <!-- 添加目录的窗口 -->
    <el-dialog :visible.sync="dialogDirectoryVisible" title="添加目录">
      <el-form
        ref="directory"
        :model="directory"
      ...
      >
        <el-form-item label="目录名称" prop="name">
          <el-input v-model="directory.name" />
        </el-form-item>
        <el-form-item label="访问路径" prop="path">
          <el-input v-model="directory.path" />
        </el-form-item>
        <el-form-item label="组件路径" prop="component">
          <el-input v-model="directory.component" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    addDirectory(){
      this.dialogDirectoryVisible=true;
    },

    //添加目录
    saveDirectory() {
    
      menu.saveLevelOne(this.directory).then((response) => {
        //关闭对话框
